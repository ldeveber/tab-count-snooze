...
describe("Windows Tab", () => {
  ...
  describe("search", () => {
    test("should search and select tabs for actions", async () => {
      ...
      const { getByLabelText, getByRole, getByText, queryByText } = renderWithContext(
        <WindowsTab />,
      );
      ...
      expect(queryByText("6 Tabs across 3 Windows")).not.toBeInTheDocument();
    ...
    test("should clear search from typing and hide/reset checkboxes", async () => {
      const user = userEvent.setup();
      ...
      const { getAllByRole, getByLabelText, getByRole, getByText, queryByRole } = renderWithContext(
        <WindowsTab />,
      );
      ...
      expect(getByRole("textbox", { name: "Search" })).toHaveValue("");
      expect(getByText("6 Tabs across 3 Windows")).toBeVisible();
      expect(getAllByRole("listitem", { name: /Tab:/ })).toHaveLength(6);

      await user.type(getByRole("textbox", { name: "Search" }), "meow");

      expect(getByRole("textbox", { name: "Search" })).toHaveValue("meow");
      expect(getAllByRole("listitem", { name: /Tab:/ })).toHaveLength(3);

      await user.clear(getByRole("textbox", { name: "Search" }));
      ...
      expect(getAllByRole("listitem", { name: /Tab:/ })).toHaveLength(6);
      expect(queryByRole("checkbox", { name: /Select tab:/ })).not.toBeInTheDocument();
    });

    test("should clear search from button and hide/reset checkboxes", async () => {
      const user = userEvent.setup();
