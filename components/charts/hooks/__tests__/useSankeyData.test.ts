...
describe("useSankeyData", () => {
  describe("_addChildren", () => {
    test("should map child data for chart", () => {
      ...
      expect(res.nodes).toContainEqual({ id: "All Tabs", depth: 0, value: tabs.length });
      ...
      expect(res.nodes).toContainEqual({ id: `${origin1}/${segment1}`, depth: 2, value: 3 });
      expect(res.nodes).toContainEqual({ id: `${origin1}/${segment2}`, depth: 2, value: 1 });
      expect(res.nodes).toContainEqual({ id: `${origin2}/${segment3}`, depth: 2, value: 1 });
      ...
      expect(res.links).toContainEqual({ source: "All Tabs", target: origin1, value: 4 });
      expect(res.links).toContainEqual({ source: "All Tabs", target: origin2, value: 1 });
