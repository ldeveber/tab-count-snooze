...
export function filterTabs(tabs: Browser.tabs.Tab[] | undefined, search: string = "") {
  if (!tabs || !tabs.length) {
    return [];
  }
  return tabs.filter(
...
export function sortTabs(tabs: Browser.tabs.Tab[], sort: SORT_OPTION) {
  if ((sort as SORT_OPTION) === SORT_OPTION.LastAccessed) {
    tabs.sort((a, b) => (a.lastAccessed && b.lastAccessed ? b.lastAccessed - a.lastAccessed : 0));
  } else {
...
export function filterSortTabs(tabs: Browser.tabs.Tab[], search: string, sort: SORT_OPTION) {
